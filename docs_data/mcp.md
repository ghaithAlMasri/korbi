...
filesystem, browser automation (for example via a Puppeteer MCP server), HTTP, or connector backed tools to an agent.
...
Another common stdio server is the Puppeteer browser automation server, which lets an agent drive a headless browser (for example, to navigate pages and extract content):
...
async with MCPServerStdio(
    ...
    agent = Agent(
        name="Assistant",
        instructions="Use the browser tools to navigate pages and extract information.",
        mcp_servers=[server],
    )
    result = await Runner.run(agent, "Open https://example.com and tell me the page title.")
    print(result.final_output)
```
