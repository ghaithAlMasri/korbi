...
Alternatively, you can supply your own pre-configured OpenAI client—useful if you need a custom base URL, proxy/network settings, retries/timeouts, or separate credentials. By default, the SDK lazily creates an `AsyncOpenAI` instance using the API key from the environment variable or the default key set above. To override the default client, use the [set_default_openai_client()][agents.set_default_openai_client] function.

```python
from openai import AsyncOpenAI
from agents import set_default_openai_client

custom_client = AsyncOpenAI(base_url_openai="...", api_key="...")
